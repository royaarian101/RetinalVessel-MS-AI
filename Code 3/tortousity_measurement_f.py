...
import math
import numpy as np
import fractal_dimension, smoothing
import math_1 as m
...
def global_cal(retina):
    ...
    width = np.sum(vessel_)/np.sum(skeleton)
...
def evaluate_window(window: Window, min_pixels_per_vessel=10, sampling_size=6, r2_threshold=0.80):  # pragma: no cover
    """
    ...
    for i in range(0, window.shape[0], 1):
        ...
        v_window = window.windows_vessel[i, 0, :, :]
        #bw_skelet=np.uint8(skeletonize(bw_window))
        vessel_total_count = np.sum(bw_window==1)
        pixel_total_count = bw_window.shape[0]*bw_window.shape[1]
        #show_images([bw_window*255,v_window*255])
        retina = Retina(v_window,bw_window)
    ...
    return FD_binary,VD_binary,Average_width, t1,t2, t4, td,vessels